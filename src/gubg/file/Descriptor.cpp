...
struct Descriptor::Pimpl: public enable_shared_from_this<Pimpl>
{
    ...
    ReturnCode selectNonBlock(EventType &et, AccessMode am)
    {
        ...
        switch (type)
        {
            case Type::File:
                ...
                break;
            default: MSS(ReturnCode::NotSupported); break;
        }
        MSS_END();
    }
    bool valid() const
    ...
    ReturnCode acceptSocket(Ptr &pp)
    {
        ...
        const auto d = ::accept(desc, (struct sockaddr *)&peer_addr, &addr_sz);
        MSS(mss::on_fail(d != -1, ReturnCode::FailedToAccept));
    ...
    {
        MSS_BEGIN(ReturnCode);
        MSS(mss::on_fail(!peer, ReturnCode::AlreadyAcceptedFile));
        const int d = ::open(fn.name().c_str(), accessMode);
        MSS(mss::on_fail(d != -1, ReturnCode::FailedToOpen));
    ...
    ReturnCode read(std::string &buffer)
    {
        ...
        L("I read " << s << " bytes");
        MSS(mss::on_fail(s >= 0, ReturnCode::FailedToRead));
        MSS_Q(mss::on_fail(s > 0, ReturnCode::PeerClosedConnection));
        MSS(s <= buffer.size());
        buffer.resize(s);
...
#endif
        L(STREAM(desc, s));
        MSS(mss::on_fail(s != -1, ReturnCode::FailedToWrite));
        MSS(s <= buffer.size());
        written = s;
...
ReturnCode Descriptor::accept(Descriptor &desc)
{
    ...
    switch (me.type)
    {
        ...
        default:           MSS(ReturnCode::UnknownType); break;
    }
    desc.pimpl_ = pp;
